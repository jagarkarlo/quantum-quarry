...
- Visual polish: Level 6 uses original animated molten surfaces/body tiles instead of tinted water. Its existing liquid-cell positions and collider outlines are preserved; earlier water levels retain their original artwork and behavior.
...
- Verified in the isolated Windows player on 2026-09-24: 243 controlled runtime checks and 21 screenshots covering all 11 scenes, including banking, vent phases/contact damage and armor rounding, pause, invisibility, death/reset, Store round trips, persistent-level transitions, Pressure HUD bounds at three resolutions, and lava/water compatibility. A real-enemy fixture also verifies terrain occlusion, stealth loss/reacquisition, and pressure-scaled perception/pursuit.
- Readability follow-up: the vent distinguishes `OFF` from armed `SAFE`, then `WARNING` and `DANGER`. The HUD explains carried-reward loss, the arming threshold, and banking to disarm; the [vent guide](../README.md#trying-the-pressure-vent) explains how to play it.
- Unresolved encounter decisions: all 15 Level 4 pickups are currently required to reach tier 2, and the pause-menu Store already provides banking without the physical detour. Evaluate these in the [human route check](DEVELOPMENT_WORKFLOW.md#remaining-human-route-check) before changing placement or economy.
